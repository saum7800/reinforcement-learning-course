...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
	...
	value_function_n = np.ones(nS)
	############################
	# YOUR IMPLEMENTATION HERE #
	while np.sum(value_function_n - value_function) > tol:
		value_function_n = value_function
		for state in range(nS):
			for probability, nextstate, reward, terminal in P[state][policy[state]]:
				value_function[state] += probability * (reward + gamma * value_function_n[nextstate])
	...
	for state in range(nS):
		...
		for action in range(nA):
			for probability, nextstate, reward, terminal in P[state][action]:
				total += probability * (reward + gamma * value_from_policy[nextstate])
			if total > max:
				max = total
